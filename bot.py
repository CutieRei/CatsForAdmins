...
initial_extension = ["cogs.event", "cogs.info", "cogs.moderator"]
...
def custom_check(ctx):
    if "Mod" in [i.name for i in ctx.author.roles] or "Reyter" in [i.name for i in ctx.author.roles] and \
            ctx.channel.name not in ["bot-1", "bot-2"]:
        return True
...
async def help(ctx):
    ...
    card.add_field(name="😎-Moderator\n",
                   ...
                   **--[ban|hammer|poop]** <member> -Ban a member\n\n--**dc** -Disconnect the bot(Reyter only)\n\n\
                   --**clear** <amount> -Clear the message of the current channel",
                   inline=False)
